...
- [x] ISC-24: Docs section states the vetted-conditioning claim and output-entropy accounting formula. Probe(J): `sed -n '2812,2815p' "${OXICRYPT_SECURITY_POLICY:-$HOME/repos/oxicrypt-policy/security-policy.md}"`; vetted claim + h_in >= n_out + 64 accounting
...
- [x] ISC-66: jent-concept mapping table for reviewer familiarity (osr→oversampling etc.). Withheld with the Security Policy from 2026-08-05 — two of its three columns cite the policy's own rule numbering. Probe(P): `grep -c 'Oversampling ratio' "$(dirname "${OXICRYPT_SECURITY_POLICY:-$HOME/repos/oxicrypt-policy/security-policy.md}")/jent-concept-mapping.md"` returns 1; content-anchored rather than line-anchored, since the old `sed -n '11,26p'` broke the moment the file gained a header; judgment: no test guards the file, and the probe needs the policy repo
...
- [x] ISC-92: Security-policy entropy section drafted from as-built gems at Phase-0 close. Probe(J): `sed -n '2790,2830p' "${OXICRYPT_SECURITY_POLICY:-$HOME/repos/oxicrypt-policy/security-policy.md}"`; 9.3.1-9.3.6 + R78-R82; nothing verifies the prose against code
...
- [ ] ISC-112: physical=false classification documented with rationale in the noise-source description. Probe(P): `sed -n '2797p' "${OXICRYPT_SECURITY_POLICY:-$HOME/repos/oxicrypt-policy/security-policy.md}"`; IG D.K R23 non-applicability; judgment: no test asserts physical==false on the wire
...
- [x] ISC-127: Security Policy entropy section states minimum entropy bits for SSP generation and per-output-bit estimate (9.3.A scenario 1 + D.J AC6). Probe(P): `sed -n '2822,2823p' "${OXICRYPT_SECURITY_POLICY:-$HOME/repos/oxicrypt-policy/security-policy.md}"`; :2823 >=384 bits; judgment: named D.J AC6 citation absent, and text carries [seeding-integration pending]
- [x] ISC-128: Known/suspected failure-mode statement documented, even if "none known" (90B §4.3 R1 + IG D.K R14; jent §6.1.42 precedent wording). Probe(P): `sed -n '2802,2810p' "${OXICRYPT_SECURITY_POLICY:-$HOME/repos/oxicrypt-policy/security-policy.md}"`; failure-mode inventory; judgment: doc-only, no test
