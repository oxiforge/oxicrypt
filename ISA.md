...
- [ ] ISC-146: `maxwell parity` exits non-zero when it did not compare everything; an all-skip or partially-skipped run is a failure unless explicitly opted out for that invocation. Probe(M): `TODO(#154)`; maxwell parity must exit non-zero when it compared nothing
