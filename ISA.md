...
- [ ] ISC-112: physical=false classification documented with rationale in the noise-source description. Probe(P): `grep -n 'D\.K R23' "${OXICRYPT_SECURITY_POLICY:-$HOME/repos/oxicrypt-policy/security-policy.md}"` matches once, inside the sentence it guards — deleting the IG D.K R23 non-applicability sentence takes it to 0; judgment: no test asserts physical==false on the wire; control: an invented resolution number returns nothing
