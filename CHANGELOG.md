...
- `acvp-harness`: `demo-run` refuses two scopes that would register more vector sets than asked for, before login so a rejected invocation costs no YubiKey touch. `--revision` without `--mode` is refused — it narrows to a revision, not to a vector set, keeping every handler that serves it across modes, so `--algorithm LMS --revision SP800-208` would register both sigGen and sigVer. A scoped `(algorithm, mode)` selection that still spans several revisions is likewise refused, naming them: registering LMS under a second revision had silently doubled the long-standing `--algorithm LMS --mode sigVer` invocation from one vector set to two. Beyond breaking one-vector-set-per-session etiquette and re-grading passed coverage, the doubling is expensive in the wrong direction — sigVer is a minutes-long session while a full-grid sigGen is an overnight tall-tree keygen run (#144).
