...
fn handle_sigver_group(group: &JsonValue) -> Result<JsonValue, DispatchError> {
    ...
    if modulo != 2048 {
        return Err(DispatchError::Unsupported(
            "RSA SigVer: only modulo 2048 is supported",
        ));
    }
    ...
    // Determine the signature verification function.
    let verify_fn: fn(&[u8; 256], u64, &[u8], &[u8; 256]) -> bool = match sig_type {
        "pkcs1v1.5" => rsa_pkcs1_verify,
        "pss" => rsa_pss_verify,
        _ => {
            return Err(DispatchError::Unsupported(
    ...
    // Parse group-level key: n (modulus, 256 bytes) and e (public exponent).
    let n_hex = group
        .get("n")
        .and_then(JsonValue::as_str)
    ...
    // Convert big-endian e bytes to u64.
    if e_bytes.len() > 8 {
    ...
    for t in tests {
        ...
        let passed = if signature.len() == 256 {
            ...
            verify_fn(&n, e_val, &message, sig)
        } else {
            // Wrong signature length → verification fails.
            false
        };
