//! Montgomery arithmetic for 4096-bit odd moduli.
//!
//! Full-width companion of the 2048-bit CRT path used for RSA-4096.
//! Given an odd modulus `n` with `2^4095 ≤ n < 2^4096`, we build a
//! [`MontCtx4096`] for CIOS Montgomery multiplication.
//!
...
define_mont_type! {
    /// Derived Montgomery constants for a specific 4096-bit odd modulus.
    pub struct MontCtx4096 for U4096;
    limbs = 64;
    bits = 4096;
}
...
mod tests {
    use super::*;
    use crate::bigint4096::LIMBS;

    fn synthetic_modulus(seed: u64) -> U4096 {
    ...
    fn mont_ctx_rejects_even_modulus() {
        ...
        n.limbs[0] &= !1u64;
        assert!(MontCtx4096::new(n).is_none());
    }
    ...
    fn mont_one_is_r_mod_n() {
        ...
        let ctx = MontCtx4096::new(n).unwrap();
    ...
    fn pow_secret_matches_pow_public_for_small_exps() {
        ...
        for exp in [0u64, 1, 2, 3, 17, 65537, 0xdead_beef] {
            ...
            let got = ctx.pow_secret(&base, &u4096_from_u64(exp));
    ...
    fn pow_secret_zero_base() {
        ...
        let ctx = MontCtx4096::new(n).unwrap();
        let big_exp = u4096_from_u64(12345);
        assert_eq!(ctx.pow_secret(&U4096::ZERO, &big_exp), U4096::ZERO);
    }
    ...
    fn pow_secret_exp_zero_is_one() {
        ...
        let ctx = MontCtx4096::new(n).unwrap();
        let r = ctx.pow_secret(&small(42), &U4096::ZERO);
        assert_eq!(r, small(1));
    }
}
