...
//!
//! For Hash_DRBG (SP 800-90A §10.1.1) we exercise each approved SHA-2
//! digest (SHA-256, SHA-384, SHA-512) with `instantiate` + two
//! `generate` calls, which also drives `Hash_df` (§10.3.1) and the
//! `Hashgen` update path.
//!
//! That's 9 KATs total. FIPS 140-3 IG 10.3.A requires exercising
...
//! Hash_DRBG KATs cover a separate SP 800-90A mechanism entirely.
