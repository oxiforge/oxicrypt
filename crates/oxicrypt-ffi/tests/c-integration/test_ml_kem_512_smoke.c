/* ML-KEM-512 C ABI smoke test — keygen / encapsulate / decapsulate
 * round-trip with implicit-rejection oracle on tamper.
 *
 * FIPS 203 §6.1–§6.3, parameter set k=2. Byte counts: d=z=m=32,
 * ek=800, dk=1632, ct=768, ss=32.
 *
 * Profile must be Unrestricted (0): ML-KEM-512 is NOT in either the
 * CNSA 1.0 transition allow-list or the CNSA 2.0 mandate (which pins
 * ML-KEM-1024). See lib.rs ml_kem_512 section comment.
 *
...
int main(void) {
    ...
    uint8_t ek[800];
    uint8_t dk[1632];
    int rc = oxi_ml_kem_512_keygen(d, z, ek, dk);
    if (rc != 0) {
    ...
    uint8_t ss_a[32];
    uint8_t ct[768];
    rc = oxi_ml_kem_512_encapsulate(ek, m, ss_a, ct);
    if (rc != 0) {
    ...
    uint8_t ss_b[32];
    rc = oxi_ml_kem_512_decapsulate(dk, ct, ss_b);
    if (rc != 0) {
    ...
    ct_bad[0] ^= 0xFF;
    uint8_t ss_bad[32];
    rc = oxi_ml_kem_512_decapsulate(dk, ct_bad, ss_bad);
    if (rc != 0) {
    ...
    printf("test_ml_kem_512_smoke: OK (keygen + encaps + decaps + implicit-rejection)\n");
    return 0;
}
