...
> Prebuilt shared and static libraries with `oxicrypt.h` are attached to each
> [GitHub release](https://github.com/oxiforge/oxicrypt/releases), so consuming the
> module from C needs no Rust toolchain.

## Documentation

**[API documentation on docs.rs](https://docs.rs/oxicrypt-ffi)** — every public item,
with the standard each one implements.

## Installing

```sh
cargo add oxicrypt-ffi
```
