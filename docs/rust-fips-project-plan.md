...
- Generator tooling in `tools/acvp-gen/` emits `crates/oxicrypt-test-vectors/src/generated.rs` from the vendored vectors, cross-validated against Python reference implementations.
...
oxicrypt/
├── crates/
│   ├── oxicrypt-module/          # Module boundary, state machine, self-tests
│   ├── oxicrypt-aes/             # AES-ECB, CBC, CTR, GCM, CCM, KW, KWP
│   ├── oxicrypt-sha/             # SHA-1 (legacy), SHA-2 (224/256/384/512), SHA-3, SHAKE
│   ├── oxicrypt-hmac/            # HMAC (all SHA variants)
│   ├── oxicrypt-drbg/            # CTR_DRBG, HMAC_DRBG, Hash_DRBG
│   ├── oxicrypt-rsa/             # RSA keygen, PKCS#1 v1.5, PSS, OAEP
│   ├── oxicrypt-ecdsa/           # ECDSA sign/verify (P-256, P-384, P-521)
│   ├── oxicrypt-ecdh/            # ECDH (SP 800-56Ar3)
│   ├── oxicrypt-eddsa/           # Ed25519, Ed448 (FIPS 186-5)
│   ├── oxicrypt-kdf/             # SP 800-108 KDF, SP 800-56C, HKDF, PBKDF2 (conditional)
│   ├── oxicrypt-xof/             # SHAKE, cSHAKE, KMAC, TupleHash, ParallelHash
│   ├── oxicrypt-cmac/            # AES-CMAC
│   └── oxicrypt-tls-kdf/         # TLS 1.2/1.3 KDF (SP 800-135)
...
The `oxicrypt-module` crate defines the cryptographic boundary per FIPS 140-3 Section 7.2:
...
|----------|--------|------------------------|
| SP 800-140A | Module types & boundaries | Software module, logical boundary defined in `oxicrypt-module` crate |
...
- DRBG health tests per SP 800-90A §11.3 (instantiate/generate/reseed
  ...
  wired as three power-up KATs in `oxicrypt-drbg::health`
- Continuous Random Number Generator Test (CRNGT) — **not applicable**
  ...
  bundle a noise source, but per §4.4 below oxicrypt consumes
...
- [x] `oxicrypt-module`: State machine, self-test runner, approved-mode indicator
- [x] `oxicrypt-aes`: AES-128/192/256 in ECB, CBC, CTR modes
- [x] `oxicrypt-aes`: AES-GCM (GHASH + CTR combination)
- [x] `oxicrypt-sha`: SHA-1, SHA-2 family (SHA-224, SHA-256, SHA-384, SHA-512, SHA-512/224, SHA-512/256)
- [x] `oxicrypt-hmac`: HMAC for all SHA variants (11 approved hash variants, including SHA-3)
- [x] Power-up KATs for Phase 1 hash/MAC/KDF/AES algorithms
- [x] Software integrity self-test mechanism (`oxicrypt-integrity`: HMAC-SHA-256 over `current_exe()` with an embedded `[HDR | MAC | FTR]` slot populated by the external `oxicrypt-integrity-sign` tool, IG 10.3.A). Uniform mechanism across desktop, server, and code-signed mobile bundles — no sidecar files and no platform-specific ELF/Mach-O/PE parsing.

### Phase 2: Asymmetric + DRBG (Weeks 5–10)

**Goal:** All P0 asymmetric algorithms and random number generation

- [x] `oxicrypt-drbg`: CTR_DRBG (AES-128/192/256, no df + use df)
- [x] `oxicrypt-drbg`: Hash_DRBG (SHA-256/384/512, §10.3.1 Hash_df)
- [x] `oxicrypt-drbg`: HMAC_DRBG (SHA-256/384/512)
- [x] `oxicrypt-drbg`: SP 800-90A §11.3 error-path health tests (CTR/Hash/HMAC_DRBG)
- [x] `oxicrypt-drbg`: SP 800-90A §9.3 prediction-resistance generate API (CTR/Hash/HMAC_DRBG); power-up KATs pending vendoring of `drbgvectors_pr_true`
- [~] CRNGT on DRBG output — deferred as not required by FIPS 140-3 IG D.G (March 2026); see §4.2 note
- [x] `oxicrypt-rsa`: PKCS#1 v1.5 verify and sign (non-CRT, constant-time windowed Montgomery ladder)
- [x] `oxicrypt-rsa`: RSASSA-PSS SHA-256 sign/verify with MGF1
- [x] `oxicrypt-rsa`: Key generation via FIPS 186-5 §A.1.1 / §B.3.1 probable primes + DRBG-backed `sign_pss_sha256` wrapper
- [x] `oxicrypt-rsa`: CRT-form private keys with Bellcore/Shamir verify-after-sign fault-detection (R5 — Garner recombine over mont1024, wired through PKCS#1 v1.5 and PSS sign entry points, with a byte-exact CRT↔non-CRT equivalence test and a dP-tamper fault-injection test)
- [~] `oxicrypt-ecdsa`: P-256/SHA-256 keygen (DRBG-backed via FIPS 186-5 §A.2.2 rejection sampler, R7), caller-`k` and DRBG-backed random-`k` sign, verify all landed; P-384 and P-521 deferred
- [x] `oxicrypt-ecdh`: ECDH per SP 800-56Ar3 (P-256)
...
- [x] `oxicrypt-aes`: AES-CCM
- [x] `oxicrypt-aes`: AES-KW, AES-KWP
- [x] `oxicrypt-cmac`: AES-CMAC
- [x] `oxicrypt-sha`: SHA-3 family, SHAKE128, SHAKE256 (SHAKE lives in `oxicrypt-xof`)
- [~] `oxicrypt-eddsa`: Ed25519 landed (R9 — deterministic RFC 8032 sign/verify per FIPS 186-5 §7.8, DRBG-backed `Ed25519PrivateKey` handle with IG 10.3.A sign-and-verify PCT, non-cofactored verify equation, canonical-`S` rejection, ct-validation target `eddsa_ed25519_scalar_mul` CLEAN at 300k worst |t|=1.418 crop=0.900); Ed448 deferred
- [x] `oxicrypt-kdf`: SP 800-108r1 (Counter, Feedback, Double-Pipeline Iteration modes), SP 800-56Cr2 (HKDF KAT retrofit); HKDF (RFC 5869) over all 11 HMACs
- [~] `oxicrypt-tls-kdf`: TLS 1.2 PRF landed (R22 — RFC 5246 §5 `P_hash` expansion, RFC 7627 EMS derivation, SHA2-{256,384,512}); TLS 1.3 HKDF deferred
- [x] `oxicrypt-rsa`: OAEP (R6 — RFC 8017 §7.1 RSAES-OAEP with SHA-256/MGF1-SHA-256, Manger-resistant decode with single-accumulator structural checks, CRT decrypt path sharing the Bellcore-protected private-exponent primitive with CRT sign, DRBG-backed `rsa_oaep_encrypt_2048_sha256` entry point, `RsaPrivateKey2048::decrypt_oaep_sha256` method)
- [x] ACVP harness: scaffolding + power-up KAT runner (122 KATs wired, including §9.3 PR DRBG KATs)
- [~] ACVP harness: vector-set dispatch (R10 — hand-rolled in-tree JSON parser with bounded-depth recursive descent, typed envelope layer over `algorithm`/`revision`/`testGroups`, `AlgorithmHandler` trait + registry, `acvp-harness dispatch <prompt.json> <response.json>` CLI subcommand gated on `require_operational`, two algorithms wired end-to-end at R10: SHA3-256 revision 2.0 AFT and HMAC-SHA2-256 revision 1.0 AFT; zero third-party dependencies preserved — no `serde_json`). R12-A expanded the ACVP handler set to seventeen AFT dispatchers total by landing SHA3-{224,384,512} revision 2.0, SHAKE-{128,256} revision FIPS202, HMAC-SHA-1 revision 1.0, HMAC-SHA2-{224,384,512,512/224,512/256} revision 1.0, and HMAC-SHA3-{224,256,384,512} revision 1.0, all sharing the R10 envelope layer with no new JSON shapes; each new family collapses into a single module (`handlers/sha3.rs`, `handlers/shake.rs`, `handlers/hmac.rs`) with a private shared group driver. R12-B then landed the **second envelope shape** R11′ promised: a hand-rolled CAVP SHS `.rsp` parser (`acvp-harness/src/rsp.rs`) over byte-oriented short-message vectors, a parallel `ShsHandler` trait + `ShsRegistry` + `process_shs` dispatcher (`acvp-harness/src/shs.rs`), seven handlers for the SHA-1 / SHA-2 family (`handlers/shs.rs` — SHA-1, SHA-224, SHA-256, SHA-384, SHA-512, SHA-512/224, SHA-512/256), and a new `acvp-harness dispatch-shs <algorithm> <prompt.rsp> <response.json>` CLI subcommand, all gated on the same `require_operational` re-check. Data-driven round-trip integration tests in `acvp-harness/tests/round_trip.rs` (eighteen tests — seventeen ACVP handlers plus an envelope-preservation test) and `acvp-harness/tests/shs_round_trip.rs` (seven tests — one per vendored `ShortMsg.rsp` file) assert every produced `md` / `mac` matches the vendored expected value byte-for-byte. Zero third-party dependencies preserved across both envelope shapes. R13 then landed the first KDF family handler, `KDA-HKDF-Sp800-56Cr2`, which is also the first ACVP family in oxicrypt that keys dispatch on the optional `mode` slot: the envelope publishes across two top-level fields (`algorithm = "KDA"`, `mode = "HKDF"`) rather than just `algorithm`. Instead of hard-coding a `KDA-HKDF` composite string, R13 generalised [`dispatch::Registry::find`] to key on `(algorithm, Option<&str>, revision)` and added a default `AlgorithmHandler::mode() -> Option<&'static str>` that returns `None`, so every R10/R12-A/R12-B handler slots into the same three-tuple without churn; [`envelope::VectorSet::mode`] exposes the optional field off the prompt, and [`dispatch::process`] conditionally threads it through the response envelope. The `handlers/kda_hkdf.rs` driver implements SP 800-56C Rev 2 §5.9.2's hybrid two-step KDF (`IKM = Z || T`, `PRK = HMAC(salt, IKM)` HKDF-Extract, fixedInfo per §5.8 pattern grammar, `OKM = HKDF-Expand(PRK, fixedInfo, L/8)`) and supports the ten HMAC instantiations legal under SP 800-56Cr2 — `SHA2-{224, 256, 384, 512, 512/224, 512/256}` and `SHA3-{224, 256, 384, 512}`, explicitly rejecting SHA-1 — reusing the existing `fips_kdf::Hkdf*` type aliases. The `fixedInfoPattern` encoder supports `uPartyInfo`, `vPartyInfo`, `l` (32-bit big-endian in bits), `algorithmId`, `context`, `label`, and `literal[HEX]` tokens per §5.8; unsupported tokens error loudly rather than silently drop bytes. A nineteenth round-trip test (`kda_hkdf_aft_round_trip`) reproduces every vendored `dkm` field byte-for-byte across all ten groups in `KDA-HKDF-Sp800-56Cr2/kat-slice.json`. `JsonValue` also gained an `as_bool` accessor to read `usesHybridSharedSecret` / `multiExpansion`. Zero third-party dependencies preserved. R14-A then landed the first symmetric-cipher AFT handlers — `ACVP-AES-{ECB, CBC, CTR}-1.0` (`handlers/aes.rs`) — bringing the registered handler count to twenty-one. Each AFT group declares a `direction` (`encrypt` / `decrypt`) and a `keyLen` (128 / 192 / 256); the handler dispatches via monomorphised `run_mode<B: BlockCipher>` to avoid boxing per-test. CTR tests with non-byte-aligned `payloadLen` are explicitly rejected as `Unsupported`; MCT groups are rejected as `UnsupportedTestType`. Three new vendored slices (`ACVP-AES-{ECB, CBC, CTR}-1.0/kat-slice.json`, six groups × three tests = eighteen tests each) and three round-trip tests (`aes_{ecb,cbc,ctr}_aft_round_trip`) verify every produced `ct` / `pt` matches byte-for-byte, now totalling twenty-two ACVP round-trip tests. R14-B then completed the AES mode set with `ACVP-AES-{GCM, CCM, KW, KWP}-1.0` AFT handlers (`handlers/aes.rs`), bringing the total to twenty-five registered handlers. GCM is filtered for the Phase-1 constraint of 96-bit IV + 128-bit tag; CCM supports all seven valid tag lengths (4–16 bytes) and nonce lengths (7–13 bytes). AEAD decrypt / key-unwrap tests carry `testPassed` fields indicating whether tag/ICV verification should succeed or fail — the handler catches `ModeError::TagMismatch` and returns `{tcId, testPassed: false}` for expected failures. Four new vendored slices and four round-trip tests (`aes_{gcm,ccm,kw,kwp}_aft_round_trip`) now total twenty-six ACVP round-trip tests. R15 then landed the MCT (Monte Carlo Test) engine for ECB and CBC (`handle_aes_mct_group`, `mct_ecb`, `mct_cbc` in `handlers/aes.rs`), implementing the standard 100×1000 iteration loop with per-key-size key-schedule update (128-bit: XOR Output[999]; 192-bit: XOR last-8-bytes(Output[998]) || Output[999]; 256-bit: XOR Output[998] || Output[999]) and direction-aware CBC IV feedback (encrypt IV = CT[j], decrypt IV = CT[j] which is the input, not the output). Six vendored MCT groups per mode (3 key sizes × 2 directions) are trimmed to 5 resultsArray entries; two new round-trip tests (`aes_{ecb,cbc}_mct_round_trip`) compare the handler's output against the vendored reference entries, now totalling twenty-eight ACVP round-trip tests. R16 then adds `CMAC-AES` revision `1.0` (SP 800-38B) with `gen` (compute) and `ver` (verify / `testPassed`) directions over all three AES key sizes, as the twenty-sixth registered handler and twenty-ninth ACVP round-trip test. R17 adds the three DRBG family handlers — `ctrDRBG` (AES-128/192/256, ±DF, ±PR), `hashDRBG` (SHA2-256/384/512, ±PR), and `hmacDRBG` (SHA2-256/384/512, ±PR) — all revision `1.0`, reaching twenty-nine registered handlers and thirty-two ACVP round-trip tests. Each handler parses group-level `mode`, `derFunc`, and `predResistance` fields, then walks the per-test `otherInput` array to sequence instantiate / reSeed / generate operations; for prediction-resistance groups, each `generate` step carries its own ACVP-supplied entropy. CTR_DRBG no-df tests build seed_material by XORing entropy with the personalization string per SP 800-90A §10.2.1.3.1. `MAX_DF_INPUT`, `HASH_DRBG_MAX_DF_INPUT`, and `HMAC_DRBG_MAX_PROVIDED` were increased to accommodate the full ACVP vector payloads (up to 640 bytes for SHA2-512). R18 adds five asymmetric signature-verification and key-validation handlers — `ECDSA` sigVer + keyVer (P-256/SHA2-256, revision `FIPS186-5`), `EDDSA` sigVer + keyVer (ED-25519, pure, revision `1.0`), and `RSA` sigVer (RSA-2048/PKCS#1v1.5/SHA2-256, revision `FIPS186-5`, GDT test type) — reaching thirty-four registered handlers and thirty-seven ACVP round-trip tests plus seven CAVP SHS tests. All five are mode-keyed (`algorithm, mode, revision`) — ECDSA uses `sigVer`/`keyVer`, EDDSA uses `sigVer`/`keyVer`, RSA uses `sigVer`. The `assert_bool_round_trip` test helper generalises verification tests: strip `testPassed` and `reason`, dispatch, compare booleans. R19 adds two SigGen handlers — `ECDSA` sigGen (P-256/SHA2-256, deterministic via caller-supplied `k`) and `EDDSA` sigGen (ED-25519, pure, naturally deterministic) — reaching thirty-six registered handlers and thirty-nine ACVP round-trip tests. ECDSA SigGen uses `sign_with_k` with the group-level private key `d` and per-test nonce `k`; Ed25519 SigGen uses `ed25519::sign(seed, message)` with the group-level seed `d`. R20 adds the SP 800-108r1 KBKDF handler (`KDF` revision `1.0`) covering all three SP 800-108 derivation modes — counter (`counterLocation = "before fixed data"`, `counterLength = 32`), feedback (`zeroLengthIv = false`, with per-test IV), and double-pipeline iteration — across all eleven HMAC instantiations (SHA-1, SHA-2 ×6, SHA-3 ×4), reaching thirty-seven registered handlers and forty ACVP round-trip tests. R21 adds the RSA DecryptionPrimitive handler (`RSA` / `decryptionPrimitive` / `Sp800-56Br2`) implementing the raw RSADP primitive with SP 800-56Br2 §7.1.2.1 range check (`1 < c < (n − 1)`), supporting both `keyMode = "standard"` (non-CRT, `n`/`d`) and `keyMode = "crt"` (CRT with Bellcore verify-after-decrypt per IG D.G, `n`/`e`/`p`/`q`/`dp`/`dq`/`qinv`), reaching thirty-eight registered handlers and forty-one ACVP round-trip tests. R22 implements the TLS 1.2 PRF (RFC 5246 §5, `P_hash` iterated-HMAC expansion) in `oxicrypt-tls-kdf` — the crate's first real algorithm code replacing its Phase 1 scaffold — and wires the `TLS-v1.2` / `KDF` / `RFC7627` Extended Master Secret handler covering SHA2-{256,384,512} across two `keyBlockLength` values, reaching thirty-nine registered handlers and forty-two ACVP round-trip tests. R23 adds the `kdf-components` / `tls` / `1.0` handler covering the standard (non-EMS) TLS 1.2 KDF — `masterSecret = PRF(PMS, "master secret", clientHelloRandom ‖ serverHelloRandom)`, `keyBlock = PRF(MS, "key expansion", serverRandom ‖ clientRandom)` — across SHA2-{256,384,512}, reaching forty registered handlers and forty-three ACVP round-trip tests. TLS v1.0/1.1 groups are filtered because the combined MD5+SHA-1 PRF requires MD5 which is not FIPS-approved. R24 adds the RSA SignaturePrimitive handler (`RSA` / `signaturePrimitive` / `2.0`) implementing RSASP1 (RFC 8017 §5.2.1) with range check `0 ≤ msg < n`, supporting both non-CRT (d-only) and CRT (Bellcore-protected, IG D.G) key formats, reaching forty-one registered handlers and forty-four ACVP round-trip tests. R25 adds RSA SigGen (`RSA` / `sigGen` / `FIPS186-5`) covering both PKCS#1v1.5 (non-CRT, d-only) and PSS (CRT with Bellcore verify-after-sign per IG D.G, caller-supplied 32-byte salt) over SHA2-256 / RSA-2048, reaching forty-two registered handlers and forty-five ACVP round-trip tests. R25 also extends the existing RSA SigVer handler with PSS groups (`pss-kat-slice.json`) containing both valid and tampered (bit-flipped) signatures, reaching forty-six ACVP round-trip tests. R26 adds the KAS-ECC-SSC handler (`KAS-ECC-SSC` / `Component` / `Sp800-56Ar3`) covering P-256 ECDH shared secret computation per SP 800-56Ar3 §5.7.1.2, reaching forty-three registered handlers and forty-seven ACVP round-trip tests. R27 adds the RSA OAEP handler (`RSA` / `OAEP` / `RFC8017`) covering RSAES-OAEP encrypt (RFC 8017 §7.1.1, deterministic with caller-supplied 32-byte seed for testability) and decrypt (§7.1.2, non-CRT) over RSA-2048 / SHA2-256 with empty label, reaching forty-four registered handlers and forty-eight ACVP round-trip tests. R28 adds EdDSA KeyGen (`EDDSA` / `keyGen` / `1.0`) deriving the Ed25519 public key from a 32-byte seed via `keygen_internal`, reaching forty-five registered handlers and forty-nine ACVP round-trip tests. R29 adds ECDSA KeyGen (`ECDSA` / `keyGen` / `FIPS186-5`) deriving the P-256 public key `(qx, qy)` from a 32-byte scalar `d` via `derive_public_key_internal`, reaching forty-six registered handlers and fifty ACVP round-trip tests. R30 adds SHA-3 Monte Carlo Test (MCT) support for all four SHA-3 variants (SHA3-224, SHA3-256, SHA3-384, SHA3-512), implementing the ACVP SHA-3 §6.2 inner loop (100×1000 chained-hash iterations) with self-generated MCT vectors (5 resultsArray entries per variant), reaching fifty-four ACVP round-trip tests. R31 extends the RSA OAEP handler with CRT decrypt support (`keyMode = "crt"`) dispatching through `rsa_oaep_decrypt_2048_sha256_crt_internal` which exercises the Bellcore-protected CRT private-exponent primitive per FIPS 140-3 IG D.G, with self-generated CRT KAT vectors (2 groups: encrypt + CRT decrypt, 5 tests each), reaching fifty-five ACVP round-trip tests. R32 adds RSA key generation (`RSA` / `keyGen` / `FIPS186-5`) with DRBG-seeded probable-prime generation per FIPS 186-5 §A.1.1/§B.3.1, returning all CRT components `(n, d, e, p, q, dP, dQ, qInv)`, with self-generated vectors (1 group, 5 tests), reaching forty-seven registered handlers and fifty-six ACVP round-trip tests. This completes the asymmetric keygen trifecta (ECDSA R29, EdDSA R28, RSA R32). R33 extends the RSA SigGen handler with `keyMode` dispatch to cover the full (sigType × keyMode) cross-product: `pkcs1v1.5` + `crt` (Bellcore-protected PKCS#1v1.5 CRT sign) and `pss` + `standard` (non-CRT PSS sign), with self-generated vectors (2 groups, 10 tests), reaching fifty-seven ACVP round-trip tests. R34 adds a combined RSA OAEP path-equivalence slice with three groups sharing one DRBG-generated key — encrypt, CRT decrypt (Bellcore/IG D.G), and non-CRT decrypt — where both decrypt groups decrypt the same ciphertexts, proving that both private-key paths produce identical plaintext (3 groups, 15 tests), reaching fifty-eight ACVP round-trip tests. R35 adds EdDSA lifecycle cross-validation slices — three self-generated vector files sharing the same five Ed25519 seeds across keyGen, sigGen, and sigVer modes, proving the full keyGen→sigGen→sigVer pipeline is consistent per key (keyGen: 1 group/5 tests, sigGen: 5 groups/25 tests, sigVer: 2 groups/50 tests with valid+invalid signatures), reaching sixty-one ACVP round-trip tests. R36 adds the equivalent ECDSA lifecycle cross-validation slices — three self-generated vector files sharing the same five DRBG-generated P-256 private keys across keyGen, sigGen (with DRBG-generated nonces `k`), and sigVer modes (keyGen: 1 group/5 tests, sigGen: 5 groups/25 tests, sigVer: 2 groups/50 tests with valid+tampered signatures), reaching sixty-four ACVP round-trip tests. R37 adds the equivalent RSA lifecycle cross-validation slices — two self-generated vector files sharing one DRBG-generated RSA-2048 key across sigGen (PKCS#1v1.5/standard + PSS/crt, 2 groups/10 tests) and sigVer (valid + invalid for each sig type, 4 groups/20 tests), proving the sigGen→sigVer pipeline is consistent across both padding modes and both key representations, reaching sixty-six ACVP round-trip tests. R38 adds SHA-3 Large Data Test (LDT) support for all four SHA-3 variants (SHA3-224, SHA3-256, SHA3-384, SHA3-512), implementing the ACVP LDT test type where a short content pattern is expanded via the `repeating` technique to a specified `fullLength` and hashed using the incremental `Sha3::update` API for streaming absorption without materializing the full message, with self-generated vectors (1 group/3 tests per variant, content sizes 512KB–2MB), reaching seventy ACVP round-trip tests. R39 completes the RSA lifecycle trifecta by adding a keyGen lifecycle slice sharing the same DRBG seed as the R37 sigGen/sigVer slices (1 group, 1 test), proving the full keyGen→sigGen→sigVer pipeline is consistent — all three asymmetric families (ECDSA R36, EdDSA R35, RSA R37+R39) now have complete lifecycle cross-validation, reaching seventy-one ACVP round-trip tests. R40 adds a KAS-ECC-SSC lifecycle slice reusing the same DRBG-generated P-256 private keys from the ECDSA lifecycle (R36) paired with fresh peer keys, proving that ECDSA-generated keys also work correctly for ECDH shared secret computation (1 group, 5 tests), reaching seventy-two ACVP round-trip tests. R41 adds AES encrypt-decrypt lifecycle slices for all seven modes (ECB, CBC, CTR, GCM, CCM, KW, KWP), each using a single DRBG-generated AES-256 key and proving encrypt→decrypt path consistency; authenticated/wrap modes include additional invalid-tag/ICV decrypt groups proving `testPassed = false` detection (simple modes: 2 groups/10 tests each; AEAD/wrap modes: 3 groups/15 tests each), reaching seventy-nine ACVP round-trip tests. R42 adds a CMAC-AES gen→ver lifecycle slice using a single DRBG-generated AES-256 key shared across gen (compute) and ver (verify) groups, with both valid and invalid (bit-flipped MAC) ver cases (3 groups, 15 tests), reaching eighty ACVP round-trip tests. R43 adds an RSA OAEP lifecycle slice reusing the same DRBG-generated RSA-2048 key from the RSA lifecycle (R37/R39) — encrypt + CRT decrypt + non-CRT decrypt groups (3 groups, 15 tests), proving keyGen→OAEP encrypt→decrypt pipeline consistency across both private-key representations, reaching eighty-one ACVP round-trip tests. R44 adds RSA primitive lifecycle slices — two self-generated vector files sharing the same DRBG-generated RSA-2048 key (from the RSA lifecycle R37/R39/R43) across signaturePrimitive (RSASP1, standard + CRT groups, 2 groups/10 tests) and decryptionPrimitive (RSADP, standard + CRT groups, 2 groups/10 tests), using the same input message representatives for both primitives to prove cross-handler agreement (both compute `input^d mod n`), reaching eighty-three ACVP round-trip tests. R45 adds SHAKE MCT and VOT test type coverage — the MCT handler implements the ACVP XOF MCT algorithm (draft-celi-acvp-xof §6.2, 100×1000 iterations with variable output length updated from the rightmost 16 bits of each squeeze modulo the output-length range, byte-aligned), and VOT (Variable Output Test) dispatches identically to AFT with per-test `outLen`; self-generated vectors for both SHAKE-128 and SHAKE-256 (MCT: 1 group/1 test with 5 resultsArray entries each, minOutLen=128/256, maxOutLen=4096; VOT: 1 group/5 tests each with DRBG-generated messages and varying output lengths), reaching eighty-seven ACVP round-trip tests. R46 adds SHAKE Large Data Test (LDT) support for both SHAKE-128 and SHAKE-256, implementing the ACVP LDT test type with streaming XOF absorption of repeating-pattern expanded messages (up to 2MB) using the incremental `Shake{128,256}::update` API, with per-test `outLen` for variable-length squeeze; self-generated vectors (1 group/3 tests per variant, content sizes 512KB–2MB, output lengths 32–128 bytes), reaching eighty-nine ACVP round-trip tests. The R11′ framing stands: upstream `usnistgov/ACVP-Server` ships no top-level `SHA-*`, `SHA1-*`, or `SHA2-*` vector directories at the pinned commit `3611942ea10c070dd8bc6afec5682d56c307de8a` and never has — plain FIPS 180-4 hashing is published only as CAVP byte-oriented SHS vectors, which is exactly what the R12-B second envelope consumes. There is no ACVP-Server re-pin in the path. R47 wires the four asymmetric algorithm crates — `fips_ecdsa`, `fips_eddsa`, `oxicrypt_rsa`, `fips_ecdh` — into the consolidated `POWER_UP_KATS` array in `acvp-harness/src/main.rs` by exporting `pub const KATS: &[KatEntry]` from each crate and adding all four slices to the compile-time `concat_kats` call (twelve crate slices total: `oxicrypt_sha`, `oxicrypt_xof`, `fips_hmac`, `fips_kdf`, `oxicrypt_aes`, `fips_cmac`, `oxicrypt_drbg`, `fips_integrity`, `fips_ecdsa`, `fips_eddsa`, `oxicrypt_rsa`, `fips_ecdh`); every approved algorithm in the module boundary now has its power-up KAT in the self-test inventory per FIPS 140-3 §7.10. R48 adds the missing TLS 1.2 PRF power-up KAT to `oxicrypt-tls-kdf` — `self_test()` exercises `tls12_prf_internal` with HMAC-SHA-256 against a pinned 48-byte reference value (the `P_hash` expansion of a fixed `(secret, label, seed)` triple), and the new `KATS` slice is wired into `POWER_UP_KATS` (thirteen crate slices total); every algorithm crate in the module boundary now has both a `self_test()` function and a `KATS` export. R49 implements cSHAKE128 and cSHAKE256 (SP 800-185 §3) in `oxicrypt-xof` — a `CShakeCore<RATE>` generic over the sponge rate absorbs the SP 800-185 `bytepad(encode_string(N) || encode_string(S), rate)` prefix before the message and uses the cSHAKE domain byte `0x04`; when both N and S are empty the core reduces to plain SHAKE (`0x1f`, no prefix). `CShake128` and `CShake256` public types mirror the SHAKE streaming API (`new → update → finalize → squeeze`), and one-shot `cshake128` / `cshake256` wrappers are provided. Two power-up KATs (N="", S="Email Signature", X=00 01 02 03, verified against an independent reference Keccak implementation) are added to `oxicrypt_xof::KATS` (four entries total).
- [~] ACVP harness: R55 adds nine SP 800-185 + PBKDF2 handlers — `cSHAKE-128`, `cSHAKE-256` (revision `1.0`, AFT, hex-encoded customization string S, N="" per ACVP registration), `KMAC-128`, `KMAC-256` (revision `1.0`, AFT, keyed MAC with hex-encoded S), `TupleHash-128`, `TupleHash-256` (revision `1.0`, AFT, tuple-element array with hex-encoded S), `ParallelHash-128`, `ParallelHash-256` (revision `1.0`, AFT, configurable block size B with hex-encoded S), and `PBKDF` (revision `1.0`, AFT, group-level `hmacAlg` selection across SHA-1/SHA-2/SHA-3 HMAC instantiations per SP 800-132) — reaching fifty-six registered ACVP handlers and ninety-eight ACVP round-trip tests. All vectors are self-generated because the NIST ACVP-Server at the pinned commit ships no cSHAKE/KMAC/TupleHash/ParallelHash/PBKDF vector directories. Handler modules: `handlers/cshake.rs`, `handlers/kmac.rs`, `handlers/tuplehash.rs`, `handlers/parallelhash.rs`, `handlers/pbkdf2.rs`. Vector generator: `tests/gen_sp800_185_slices.rs` (eight slices), `tests/gen_pbkdf2_slice.rs` (one slice with six hmacAlg groups). Zero third-party dependencies preserved.
- [~] ACVP harness: R56 adds six SP 800-185 XOF variant handlers — `KMACXOF-128`, `KMACXOF-256` (revision `1.0`, AFT, keyed XOF via `finalize()` + `squeeze()` with hex-encoded customization string S), `TupleHashXOF-128`, `TupleHashXOF-256` (revision `1.0`, AFT, tuple-element array with XOF squeeze output), `ParallelHashXOF-128`, `ParallelHashXOF-256` (revision `1.0`, AFT, configurable block size B with XOF output) — reaching sixty-two registered ACVP handlers and one hundred four ACVP round-trip tests. The XOF handlers reuse the shared group drivers from their non-XOF counterparts (`handle_kmac_group`, `handle_tuplehash_group`, `handle_parallelhash_group`), differing only in the crypto closure (squeeze pattern vs. `finalize_into`). All vectors self-generated. Vector generator: `tests/gen_sp800_185_xof_slices.rs` (six slices). Zero third-party dependencies preserved.
- [~] ACVP harness: R57 adds KMAC / KMACXOF MVT (MAC Verification Test) support — all four KMAC family handlers (`KMAC-128`, `KMAC-256`, `KMACXOF-128`, `KMACXOF-256`) now accept both `testType = "AFT"` (compute MAC) and `testType = "MVT"` (verify MAC, return `testPassed`). The shared `handle_kmac_group` driver was refactored to dispatch on test type, with per-test input parsing extracted into `parse_kmac_test` to satisfy clippy's `too_many_lines` lint. Self-generated MVT vectors include both valid-MAC and bit-flipped-MAC groups (2 groups × 5 tests per algorithm, 4 algorithms). Reaching sixty-two registered ACVP handlers and one hundred eight ACVP round-trip tests. Vector generator: `tests/gen_kmac_mvt_slices.rs`. Zero third-party dependencies preserved.
...
- [~] `oxicrypt-xof`: cSHAKE landed (R49 — SP 800-185 §3, CShake128/CShake256 with bytepad prefix, domain 0x04, empty-N-S fallback to SHAKE); KMAC128/KMAC256 landed (R50 — SP 800-185 §4, KmacCore<RATE> generic absorbs `bytepad(encode_string(K), rate)` then message then `right_encode(L)`, finalized through cSHAKE with N="KMAC"; streaming `Kmac{128,256}` types + one-shot `kmac{128,256}` wrappers; two power-up KATs from NIST KMAC_samples.pdf — Sample #1 KMAC128 and Sample #5 KMAC256; `oxicrypt_xof::KATS` now 6 entries); KMACXOF128/KMACXOF256 landed (R51 — SP 800-185 §4.3.1, XOF variant with `right_encode(0)`, streaming `KmacXof{128,256}` types + one-shot `kmacxof{128,256}` wrappers; two power-up KATs cross-checked against pycryptodome cSHAKE_XOF; `oxicrypt_xof::KATS` now 8 entries); TupleHash128/TupleHash256 + TupleHashXOF128/TupleHashXOF256 landed (R52 — SP 800-185 §5/§5.3.1, TupleHashCore<RATE> wraps each update element with encode_string for unambiguous tuple framing, fixed-output and XOF variants; two power-up KATs from SP 800-185 §A.3/§A.4 Sample #1; `oxicrypt_xof::KATS` now 10 entries); ParallelHash128/ParallelHash256 + ParallelHashXOF128/ParallelHashXOF256 landed (R53 — SP 800-185 §6/§6.3.1, ParallelHashCore<RATE, INNER_LEN> splits input into B-byte blocks, hashes each through inner SHAKE to INNER_LEN bytes, feeds `left_encode(B) || inner_hashes || right_encode(n) || right_encode(L)` through outer cSHAKE with N="ParallelHash"; streaming API buffers up to B bytes; XOF variant uses `right_encode(0)`; ParallelHash128 KAT from NIST ParallelHash_samples.pdf Sample #1, ParallelHash256 KAT cross-checked against pycryptodome; `oxicrypt_xof::KATS` now 12 entries; 55 unit tests pass)
- [x] PBKDF2 (R54 — SP 800-132 / RFC 8018 §5.2 in `oxicrypt-kdf`, `Pbkdf2<P, L>` generic over PrfHmac, 11 type aliases covering all approved HMACs, two power-up KATs: PBKDF2-HMAC-SHA-1 from RFC 6070 Test Case 1 and PBKDF2-HMAC-SHA-256 cross-checked via Python hashlib; `fips_kdf::KATS` now 46 entries)
