...
- [~] ACVP harness: vector-set dispatch (R10 — hand-rolled in-tree JSON parser with bounded-depth recursive descent, typed envelope layer over `algorithm`/`revision`/`testGroups`, `AlgorithmHandler` trait + registry, `acvp-harness dispatch <prompt.json> <response.json>` CLI subcommand gated on `require_operational`, two algorithms wired end-to-end at R10: SHA3-256 revision 2.0 AFT and HMAC-SHA2-256 revision 1.0 AFT; zero third-party dependencies preserved — no `serde_json`). R12-A expanded the handler set to seventeen AFT dispatchers total by landing SHA3-{224,384,512} revision 2.0, SHAKE-{128,256} revision FIPS202, HMAC-SHA-1 revision 1.0, HMAC-SHA2-{224,384,512,512/224,512/256} revision 1.0, and HMAC-SHA3-{224,256,384,512} revision 1.0, all sharing the R10 envelope layer with no new JSON shapes. Each new family collapses into a single module (`handlers/sha3.rs`, `handlers/shake.rs`, `handlers/hmac.rs`) with a private shared group driver, and each new handler gets a data-driven round-trip integration test that loads the vendored `internalProjection` slice, strips the answer field, dispatches, and asserts the response matches byte-for-byte — eighteen round-trip tests pass end-to-end (seventeen handlers plus the envelope-preservation test). Remaining: HKDF, AES, DRBG, ECDSA, EdDSA, RSA handler families, plus MCT/LDT test types. SHA-2 dispatch will not come from ACVP-Server `internalProjection` at all: upstream `usnistgov/ACVP-Server` ships no top-level `SHA-*`, `SHA1-*`, or `SHA2-*` vector directories at the pinned commit `3611942ea10c070dd8bc6afec5682d56c307de8a` (= master HEAD as of this writing) and never has — plain FIPS 180-4 hashing is published only as CAVP byte-oriented SHS vectors, which is what the `[cavp_shs]` MANIFEST section already vendors. A SHA-2 handler family therefore needs a second envelope shape over the CAVP SHS layout rather than a workspace-wide ACVP-Server re-pin; the earlier deferral note wrongly framed it as a pin-bump blocker and has been retired.
